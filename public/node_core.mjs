...
export default class NodeCore {
  ...
  isInputConnected(slot) {
    return (
      slot < NodeCore.getInputs(this.properties).length &&
      NodeCore.getInputs(this.properties)[slot].links.length > 0
    );
  }
  ...
  getOutputNodes(slot) {
    if (
      ...
      NodeCore.getOutputs(this.properties).length == 0
    ) {
      return null;
    }
  ...
  findOutputSlot(name, returnObj) {
    ...
    for (
      var i = 0, l = NodeCore.getOutputs(this.properties).length;
      i < l;
      ++i
    ) {
