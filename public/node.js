...
export default class LGraphNode extends NodeCore {
  ...
  setSize(size, update = true) {
    ...
    if (update)
      window.sendToServer("setSize", { nodeID: this.id, size: this.size });
  }
  /**
  ...
  removeOutput(slot) {
    ...
    for (var i = slot; i < NodeCore.getOutputs(this.properties).length; ++i) {
      if (
        !NodeCore.getOutputs(this.properties)[i] ||
        !NodeCore.getOutputs(this.properties)[i].links
      ) {
        continue;
      }
  ...
  removeInputByName(name) {
    ...
    let slot = NodeCore.getInputs(this.properties).findIndex(
      (item) => item.name === name
    );
    if (slot >= 0) {
  ...
  removeOutputByName(name) {
    ...
    let slot = NodeCore.getOutputs(this.properties).findIndex(
      (item) => item.name === name
    );
    if (slot >= 0) {
  ...
  computeSize(props) {
    ...
    var rows = Math.max(
      NodeCore.getInputs(props).length,
      NodeCore.getInputs(props).length
    );
    if (rows < 1) rows = 1;
    var size = [0, 0];
    ...
    size[1] =
      ...
      rows * window.LiteGraph.NODE_SLOT_HEIGHT;
  ...
  addWidget(type, name, value, callback, options) {
    ...
    if (!callback && !w.options.callback && !w.options.property) {
      console.warn(
        "window.LiteGraph addWidget(...) without a callback or property assigned"
      );
    }
  ...
  getSlotInPosition(x, y) {
    ...
    if (NodeCore.getInputs(this.properties)) {
      for (
        var i = 0, l = NodeCore.getInputs(this.properties).length;
        i < l;
        ++i
      ) {
        var input = NodeCore.getInputs(this.properties)[i];
    ...
    if (NodeCore.getOutputs(this.properties)) {
      for (
        let i = 0, l = NodeCore.getOutputs(this.properties).length;
        i < l;
        ++i
      ) {
        var output = NodeCore.getOutputs(this.properties)[i];
  ...
  getConnectionPos(is_input, slot_number, out) {
    ...
    if (
      ...
      NodeCore.getOutputs(this.properties)[slot_number].pos
    ) {
      out[0] =
        this.pos[0] + NodeCore.getOutputs(this.properties)[slot_number].pos[0];
      out[1] =
        this.pos[1] + NodeCore.getOutputs(this.properties)[slot_number].pos[1];
      return out;
    }
    ...
    out[1] =
      this.pos[1] +
      (slot_number + 0.5) * window.LiteGraph.NODE_SLOT_HEIGHT +
      (this.constructor.slot_start_y || 0);
    return out;
  }
  /* Force align to grid */
  alignToGrid() {
    ...
    if (this.size[0] >= gridSize) {
      ...
      this.pos[0] =
        gridSize * (Math.round(this.pos[0] / gridSize) + 0.5) -
        this.size[0] / 2;
    }
    if (this.size[1] >= gridSize) {
      ...
      this.pos[1] =
        gridSize * (Math.round(this.pos[1] / gridSize) + 0.5) -
        this.size[1] / 2;
    }
  }
  setPos(x, y) {
